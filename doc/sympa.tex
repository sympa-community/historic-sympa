...
\begin {document}
    ...
    \title {\Huge\bf Sympa \\ \huge\bf Mailing Lists Management Software \\ version 3.4.3\\}
    \author {
        Serge Aumont,
        Olivier Sala\"un,
...
\begin {itemize}
    ...
    \item Privileged operations can be performed by list editors or
        ...
        in the \file {}  global configuration file (listmaster
...
\begin {itemize}
	...
	page~\pageref {scenarii}. Those scenarii are default scenarii but you may look at
        \dir {/\samplerobot/scenari/} for default scenarii of \samplerobot
        virtual robot and \dir {/\samplelist/scenari} for scenarii
        specific to a particular list 

	\item \dir {/list\_task\_models/}\\
	This directory will store your own list task models (see \ref {tasks}, page~\pageref {tasks}).	

	\item \dir {/global\_task\_models/}\\
	Contains global task models of yours (see \ref {tasks}, page~\pageref {tasks}).		
	
	\item \dir {/wws\_templates/}\\
	The web interface (\WWSympa) is composed of template HTML
	files parsed by the CGI program. Templates can also 
        be defined for a particular list in \dir {/\samplelist/wws\_templates/}
        or in \dir {/\samplerobot/wws\_templates/}
	...
	\item \dir {/\samplerobot}\\
        ...
        in \dir {/\samplerobot}).
        Those directories for virtual robots have the same structure as  \dir {} which is
	...
	page~\pageref {list-directory}). Lists stored in this directory
        ...
        can be stored in \dir {/\samplerobot/\samplelist} directory and it
        is managed by \samplerobot virtual robot.

	\item \dir {/X509-user-certs}\\
...
\begin {quote}
\begin{verbatim}
sympa:             "| /queue sympa@\samplerobot"
listmaster: 	   "| /queue listmaster@\samplerobot"
bounce+*:          "| /bouncequeue sympa@\samplerobot"
...
\begin {quote}
    ...
    \begin {tabular} {ll}
        \mailaddr {\samplelist}:         &
            "|/queue \samplelist@\samplerobot"
            \\
        \mailaddr {\samplelist-request}: &
            "|/queue \samplelist-request@\samplerobot"
            \\
        \mailaddr {\samplelist-editor}:  &
            "|/queue \samplelist-editor@\samplerobot"
            \\
        \mailaddr {\samplelist-owner}:   &
            "|/bouncequeue \samplelist@\samplerobot
            \\
        \mailaddr {\samplelist-subscribe}:   &
            "|/queue \samplelist-subscribe@\samplerobot@\samplerobot"
            \\
        \mailaddr {\samplelist-unsubscribe}: &
            "|/queue \samplelist-unsubscribe@\samplerobot"
            \\
...
as defined in the \file {/\samplelist/config}
...
\file {/\samplelist/config}.  This address definition
is not compulsory.
...
\end{enumerate}
Example : \file {/alias\_manager.pl add \samplelist cru.fr}

\file {/alias\_manager.pl} works on the alias file as defined
...
\begin {quote}
\begin{verbatim}
    \samplerobot-sympa:   "|/sympa.pl sympa@\samplerobot"
    .....
    \samplerobot-listA:   "|/sympa.pl listA@\samplerobot"
\end{verbatim}
\end {quote}
...
The \file {} configuration file contains numerous
...
The \file {} file contains directives in the following
format:
...
You may locate this file either in \begin{enumerate}
 	\item \dir {/\samplelist/mhonarc-ressources}
	in order to create a specific archive look for a particular list

	\item or \dir {/mhonarc-ressources}
...
\dir {/\samplelist}). Note that the html archive 
...
\end {quote}
     ...
     to \dir {/outgoing/}. (No need to create nonexistent subscribers to receive
     copies of messages)
...
".rebuild.\samplelist@myhost" in \dir {/outgoing}, and make sure that
the owner of this file is \Sympa. 

\begin {quote}
\begin{verbatim}
     example : su sympa -c "touch /outgoing/.rebuild.sympa-fr@cru.fr"
\end{verbatim}
\end {quote}
...
The \file {/auth.conf} configuration file contains numerous
parameters which are read on start-up of \Sympa. If you change this file, do not forget
that you will need to restart \Sympa afterwards. 

The \file {/auth.conf} is organised in paragraphs. Each paragraph coincides with the configuration of 
an ldap directory.

The \file {/auth.conf} file contains directives in the following format:
...
\subsection {Definition}
	...
	People are selected through an \index {LDAP filter} defined in a configuration file. This file must have the extension '.ldap'.It is stored in \dir {/search\_filters/}.
...
\begin{itemize}
  ...
  \item \cfkeyword {cafile} : the path of a bundle of trusted ca certificates. 
        The file \tildefile {/ca\-bundle.crt} included in Sympa distribution can be used.
...
the listmaster using \dir {/p12topem.pl} script. This script allows
...
\dir {/\samplelist/private\_key}. All the list private
...
the \unixcmd {/p12topem.pl} script. Use \unixcmd
...
certificates stored in the \dir {/X509-user-certs/} directory.
...
\begin {quote}
\begin{verbatim}
sympa@your.virtual.domain:      "| /queue sympa@your.virtual.domain"
listmaster@your.virtual.domain: "| /queue listmaster@your.virtual.domain"
\end{verbatim}
\end {quote}
...
\dir {/\samplerobot}. This directory must contain at least a 
\file {robot.conf} file. This files has the same format as  \file {}
...
These settings overwrite the equivalent global parameter defined in \file {}
...
\dir {/\samplerobot/wws\_templates/},
\dir {/\samplerobot/templates/}, 
\dir {/\samplerobot/scenari/} directories are searched when
loading templates or scenari before searching into \dir {} and  \dir {}. This allows to define different privileges and a different GUI for a Virtual Robot.
...
\begin {enumerate}
	\item \dir {/\texttt{<}list\texttt{>}/\texttt{<}action\texttt{>}.\texttt{<}lang\texttt{>}.tpl}. 
	\item \dir {/\texttt{<}list\texttt{>}/\texttt{<}action\texttt{>}.tpl}. 
	\item \dir {/templates/\texttt{<}action\texttt{>}.\texttt{<}lang\texttt{>}.tpl}. 
	\item \dir {/templates/\texttt{<}action\texttt{>}.tpl}. 
	\item \dir {/templates/\texttt{<}action\texttt{>}.\texttt{<}lang\texttt{>}.tpl}.
	\item \dir {/templates/\texttt{<}action\texttt{>}.tpl}.
\end {enumerate}
...
Your list web template files should be placed in the \dir {/\samplelist/wws\_templates} 
directory ; your site web templates in \tildedir {/wws\_templates} directory.
...
\dir {} directory. Messages have currently been
...
file, located in the \dir {} directory. The format of this file is 
as follows :
\begin {quote}
\begin{verbatim}
<topic1_name>
...
looks first in the related list directory \dir {/\texttt{<}list\texttt{>}/scenari}. If it
does not find the file there, it scans \dir {/scenari},
and finally \dir {/scenari}, which is the directory installed by the Makefile.
...
These standard scenarii are located in the \dir {/scenari/}
...
\dir {/scenari} directory. They will not be overwritten by Sympa release.
Scenarii can also be defined for a particular virtual robot (using directory \dir {/\texttt{<}robot\texttt{>}/scenari}) or for a list ( \dir {/\texttt{<}robot\texttt{>}/\texttt{<}list\texttt{>}/scenari} ).
...
\Sympa provides several task models stored in \dir {/global\_task\_models} 
  and \dir {/list\_task\_models} directories.
...
\begin {enumerate}
	\item \dir {/\texttt{<}list name\texttt{>}/}
	\item \dir {/list\_task\_models/}
	\item \dir {/list\_task\_models/}
\end {enumerate}
...
\file {/\samplerobot/\samplelist/config} 
(or \file {/\samplelist/config} if no virtual robot is defined). 
...
The \file {/\samplelist/subscribers} file is automatically created and
...
\file {/\samplelist/info} should contain a detailed text
...
\file {/\samplelist/homepage} is the HTML text 
...
\begin {enumerate}
 	\item \dir {/\samplelist/\texttt{<}file\texttt{>}.tpl} 
	\item \dir {/templates/\texttt{<}file\texttt{>}.tpl}. 
	\item \dir {/templates/\texttt{<}file\texttt{>}.tpl}.
\end {enumerate}
...
\file {/\samplelist/stats} is a text file containing 
...
You may create \file {/\samplelist/message.header} and
\file {/\samplelist/message.footer} files. Their content
...
The \dir {/\samplelist/archives/} directory contains the 
...
List profiles can be stored in \dir {/create\_list\_templates} or
\dir {/create\_list\_templates}, which are part of the Sympa
distribution and should not be modified.  
\dir {/create\_list\_templates}, which will not be
...
\dir {/create\_list\_templates} and
\dir {/create\_list\_templates} directory, then the local profile 
...
the \dir {} directory, and you may create your own customized
version in \dir {}).
...
For each parameter, you may specify (via the \file {/edit\_list.conf}
configuration file) who has the right to edit the parameter concerned ; the default 
\file {/edit\_list.conf} is reasonably safe.
...
Domain name of the list, default is the robot domain name set in the related \file {robot.conf} file or in file \file {}.
...
\item  \lparam {footer\_type} \texttt {append} 
        ...
        message. \file {/\samplelist/message.footer.mime} will be
...
they are stored in \dir {/\samplelist/archives/} directory.
...
(\dir {/\samplelist/archives/}), used to store these documents.
...
A list's shared documents are stored in the \dir {/\samplelist/shared}
directory. 
...
\begin {quote}
\begin{verbatim}
	\samplelist-owner: "|/bouncequeue \samplelist"
\end{verbatim}
\end {quote}
...
For each mail received, \Sympa extracts its MIME parts in the \dir {/tmp/antivirus} directory and
