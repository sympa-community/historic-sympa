 ...
 \item \textbf {Multilingual} user interface. The full user/admin interface (mail and web)
   ...
   \item \textbf {Template based} user interface. Every web page and service message can
    ...
    \item Privileged operations can be performed by list editors or
        ...
        in the \file {/usr/local/sympa-ff//etc/sympa.conf}  global configuration file (listmaster
...
\begin {itemize}
	...
	files as in the distribution.  \dir {/usr/local/sympa-ff//bin} may be completely
        overwritten by the \unixcmd {make install} so you must not customize
        templates and authorization scenarios under  \dir {/usr/local/sympa-ff//bin}.

	\item \dir {/usr/local/sympa-ff//bin/etc}\\
	Here \Sympa stores the default versions of what it will otherwise find
	in \dir {/usr/local/sympa-ff//etc} (task models, authorization scenarios, templates and configuration
	files, recognized S/Mime certificates, families).

	\item \dir {/usr/local/sympa-ff//etc}\\
	This is your site's configuration directory. Consult
	\dir {/usr/local/sympa-ff//bin/etc} when drawing up your own.

	\item \dir {/usr/local/sympa-ff//etc/create\_list\_templates/}\\
	List templates (suggested at list creation time).

	\item \dir {/usr/local/sympa-ff//etc/scenari/}\\
	...
	If you don't know what the hell an authorization scenario is, refer to \ref {scenarios},page~\pageref {scenarios}. Those authorization scenarios are default scenarios but you may look at
        \dir {/usr/local/sympa-ff//etc/\samplerobot/scenari/} for default scenarios of \samplerobot
        virtual robot and \dir {/usr/local/sympa-ff//expl/\samplelist/scenari} for scenarios
        specific to a particular list 

	\item \dir {/usr/local/sympa-ff//etc/data\_sources/}\\
	...
	\item \dir {/usr/local/sympa-ff//etc/list\_task\_models/}\\
	This directory will store your own list task models (see \ref {tasks}, page~\pageref {tasks}).	

	\item \dir {/usr/local/sympa-ff//etc/global\_task\_models/}\\
	Contains global task models of yours (see \ref {tasks}, page~\pageref {tasks}).		
	
	\item \dir {/usr/local/sympa-ff//etc/web\_tt2/} (used to be \dir {/usr/local/sympa-ff//etc/wws\_templates/})\\
	The web interface (\WWSympa) is composed of template HTML
	files parsed by the CGI program. Templates can also 
        be defined for a particular list in \dir {/usr/local/sympa-ff//expl/\samplelist/web\_tt2/}
        or in \dir {/usr/local/sympa-ff//etc/\samplerobot/web\_tt2/}

	\item \dir {/usr/local/sympa-ff//etc/mail\_tt2/} (used to be \dir {/usr/local/sympa-ff//etc/templates/})\\
	...
	\item \dir {/usr/local/sympa-ff//etc/families/}\\
	Contains family directories of yours (see \ref {ml-creation}, page~\pageref {ml-creation}).
	Families directories can also be created in \dir {/usr/local/sympa-ff//etc/\samplerobot/families/}

	\item \dir {/usr/local/sympa-ff//etc/\samplerobot}\\
        ...
        in \dir {/usr/local/sympa-ff//expl/\samplerobot}).
        Those directories for virtual robots have the same structure as  \dir {/usr/local/sympa-ff//etc} which is
        the configuration dir of the default robot. 

	\item \dir {/usr/local/sympa-ff//expl}\\
	\Sympa's working directory.

	\item \dir {/usr/local/sympa-ff//expl/\samplelist}\\
	...
	page~\pageref {list-directory}). Lists stored in this directory
        ...
        can be stored in \dir {/usr/local/sympa-ff//expl/\samplerobot/\samplelist} directory and it
        is managed by \samplerobot virtual robot.

	\item \dir {/usr/local/sympa-ff//expl/X509-user-certs}\\
	The directory where Sympa stores all user's certificates

	\item \dir {/usr/local/sympa-ff//locale}\\
...
%\Sympa has currently been translatedinto 14 different languages.
	...
	\item \dir {/usr/local/sympa-ff//spool}\\
	\Sympa uses 9 different spools (see \ref{spools}, page~\pageref{spools}).

	\item \dir {/usr/local/sympa-ff//src/}\\
	\Sympa sources.
...
\begin {itemize}
	...
	\item \file {/usr/local/sympa-ff//etc/sympa.conf}\\
	The main configuration file.
	See \ref{exp-admin}, page~\pageref{exp-admin}.
	

	\item \file {/usr/local/sympa-ff//etc/wwsympa.conf}\\
...
\begin {itemize}
	...
	\item \dir {/usr/local/sympa-ff//spool/auth/}\\
	For storing messages until they have been confirmed.

	\item \dir {/usr/local/sympa-ff//spool/bounce/}\\
	For storing incoming bouncing messages.

	\item \dir {/usr/local/sympa-ff//spool/digest/}\\
	For storing lists' digests before they are sent.

	\item \dir {/usr/local/sympa-ff//spool/mod/}\\
	For storing unmoderated messages.

	\item \dir {/usr/local/sympa-ff//spool/msg/}\\
	For storing incoming messages (including commands).

	\item \dir {/usr/local/sympa-ff//spool/msg/bad/}\\
	\Sympa stores rejected messages in this directory

	\item \dir {/usr/local/sympa-ff//spool/distribute/}\\
	...
	\item \dir {/usr/local/sympa-ff//spool/distribute/bad/}\\
	\Sympa stores rejected messages in this directory
	
	\item \dir {/usr/local/sympa-ff//spool/task/}\\
	For storing all created tasks.

	\item \dir {/usr/local/sympa-ff//spool/outgoing/}\\
	\file {sympa.pl} dumps messages in this spool to await archiving
	by \file {archived.pl}.

	\item \dir {/usr/local/sympa-ff//spool/topic/}\\
	For storing topic information files. 

	\item \dir {/usr/local/sympa-ff//spool/tracability/}\\
...
login directory as \dir {/usr/local/sympa-ff/}.
...
Sympa comes with default configuration files (templates, scenarios,...) that will be installed in the \dir {/usr/local/sympa-ff//bin} directory. 
If you need to customize some of them, you should copy the file first in a safe place, ie in the \dir {/usr/local/sympa-ff//etc} directory.
...
\begin {quote}
sympa:             "| /usr/local/sympa-ff//bin/queue sympa@\samplerobot"\\
listmaster: 	   "| /usr/local/sympa-ff//bin/queue listmaster@\samplerobot"\\
bounce+*:          "| /usr/local/sympa-ff//bin/bouncequeue sympa@\samplerobot"\\
...
\begin {quote}
    ...
    \begin {tabular} {ll}
        \mailaddr {\samplelist}:         &
            "|/usr/local/sympa-ff//bin/queue \samplelist@\samplerobot"
            \\
        \mailaddr {\samplelist-request}: &
            "|/usr/local/sympa-ff//bin/queue \samplelist-request@\samplerobot"
            \\
        \mailaddr {\samplelist-editor}:  &
            "|/usr/local/sympa-ff//bin/queue \samplelist-editor@\samplerobot"
            \\
        \mailaddr {\samplelist-owner}:   &
            "|/usr/local/sympa-ff//bin/bouncequeue \samplelist@\samplerobot
            \\
        \mailaddr {\samplelist-subscribe}:   &
            "|/usr/local/sympa-ff//bin/queue \samplelist-subscribe@\samplerobot@\samplerobot"
            \\
        \mailaddr {\samplelist-unsubscribe}: &
            "|/usr/local/sympa-ff//bin/queue \samplelist-unsubscribe@\samplerobot"
            \\
...
as defined in the \file {/usr/local/sympa-ff//expl/\samplelist/config}
...
\file {/usr/local/sympa-ff//expl/\samplelist/config}.  This address definition
is not compulsory.
...
\end{enumerate}
Example : \file {/usr/local/sympa-ff//bin/alias\_manager.pl add \samplelist cru.fr}

\file {/usr/local/sympa-ff//bin/alias\_manager.pl} works on the alias file as defined in \file {sympa.conf})
...
\file {/usr/local/sympa-ff//bin/alias\_manager.pl} runs a \unixcmd{newaliases} command (via \file {aliaswrapper}), after any changes to aliases file.
...
\begin {quote}
    \samplerobot-sympa:   "|/usr/local/sympa-ff//bin/sympa.pl sympa@\samplerobot"
    .....
    \samplerobot-listA:   "|/usr/local/sympa-ff//bin/sympa.pl listA@\samplerobot"
\end {quote}
...
The \file {/usr/local/sympa-ff//etc/sympa.conf} configuration file contains numerous
...
The \file {/usr/local/sympa-ff//etc/sympa.conf} file contains directives in the following
format:
...
\subsection {\cfkeyword {home}}
	 ...
	 \default {\dir {/usr/local/sympa-ff//expl}}
...
\subsection {\cfkeyword {etc}}
	 ...
	 \default {\dir {/usr/local/sympa-ff//etc}}
...
\subsection {\cfkeyword {pidfile}} 
	...
	\default {\file {/usr/local/sympa-ff//etc/sympa.pid}}
...
\subsection {\cfkeyword {spool}}
        ...
        \default {\dir {/usr/local/sympa-ff//spool}}
...
\subsection {\cfkeyword {queue}} 
	...
	\example {\dir {/usr/local/sympa-ff//spool/msg}}

\subsection {\cfkeyword {queuedistribute}} 
	\index{spool}
	\index{distribution}
	
	\default {\dir {/usr/local/sympa-ff//spool/distribute}}
...
\subsection {\cfkeyword {queuemod}}  
	...
	\default {\dir {/usr/local/sympa-ff//spool/moderation}}
...
\subsection {\cfkeyword {queueauth}} 
	...
	\default {\dir {/usr/local/sympa-ff//spool/auth}}

        This parameter is optional and retained solely for backward compatibility.

\subsection {\cfkeyword {queueoutgoing}} 

	\default {\dir {/usr/local/sympa-ff//spool/outgoing}}

	This parameter is optional and retained solely for backward compatibility.

\subsection {\cfkeyword {queuetopic}} 

	\default {\dir {/usr/local/sympa-ff//spool/topic}}
...
\subsection {\cfkeyword {queuebounce}} 
	...
	\default {\dir {/usr/local/sympa-ff//spool/bounce}}
...
\subsection {\cfkeyword {queuetask}} 
	...
	\default {\dir {/usr/local/sympa-ff//spool/task}}
...
\subsection {\cfkeyword {queuetracability}}
        ...
        \default {\dir {/usr/local/sympa-ff//spool/tracability}}

        This parameter is optional and retained solely for backward compatibility.

\subsection {\cfkeyword {tmpdir}}

        \default {\dir {/usr/local/sympa-ff//spool/tmp}}
...
\subsection {\cfkeyword {localedir}}   
	...
	\default{\dir {/usr/local/sympa-ff//locale}}
...
(
    ...
    subscribed_date_subscriber datetime           null,	
    who_init_subscriber	   varchar(100)           null,
    who_update_subscriber  varchar(100)           null,
    how_init_subscriber    varchar(4)             null,
    how_update_subscriber  varchar(4)             null,
    ip_init_subscriber     varchar(15)            null,
    ip_update_subscriber   varchar(15)            null,
...
You may locate this file either in \begin{enumerate}
 	\item \dir {/usr/local/sympa-ff//expl/\samplelist/mhonarc-ressources}
	in order to create a specific archive look for a particular list

	\item or \dir {/usr/local/sympa-ff//etc/mhonarc-ressources}
...
\dir {/usr/local/sympa-ff//expl/\samplelist}). Note that the html archive 
...
\end {quote}
     ...
     to \dir {/usr/local/sympa-ff//spool/outgoing/}. (No need to create nonexistent subscribers to receive
...
".rebuild.\samplelist@myhost" in \dir {/usr/local/sympa-ff//spool/outgoing}, and make sure that
the owner of this file is \Sympa. 

\begin {quote}
\begin{verbatim}
     example : su sympa -c "touch /usr/local/sympa-ff//spool/outgoing/.rebuild.sympa-fr@cru.fr"
\end{verbatim}
\end {quote}
...
The \file {/usr/local/sympa-ff//etc/auth.conf} configuration file contains numerous
parameters which are read on start-up of \Sympa. If you change this file, do not forget
that you will need to restart wwsympa.fcgi afterwards. 

The \file {/usr/local/sympa-ff//etc/auth.conf} is organised in paragraphs. Each paragraph describes an authentication 
...
The \file {/usr/local/sympa-ff//etc/auth.conf} file contains directives in the following format:
...
looks first in the related list directory \dir {/usr/local/sympa-ff//expl/\texttt{<}list\texttt{>}/scenari}. If it
does not find the file there, it scans the current robot configuration directory \dir {/usr/local/sympa-ff//etc/\samplerobot/scenari}, then the site's configuration directory \dir {/usr/local/sympa-ff//etc/scenari},
and finally \dir {/usr/local/sympa-ff//bin/etc/scenari}, which is the directory installed by the Makefile.
...
These standard authorization scenarios are located in the \dir {/usr/local/sympa-ff//bin/etc/scenari/}
...
\dir {/usr/local/sympa-ff//etc/scenari} directory. They will not be overwritten by Sympa release.
Scenarios can also be defined for a particular virtual robot (using directory \dir {/usr/local/sympa-ff//etc/\texttt{<}robot\texttt{>}/scenari}) or for a list ( \dir {/usr/local/sympa-ff//expl/\texttt{<}robot\texttt{>}/\texttt{<}list\texttt{>}/scenari} ).
...
\subsection {Definition}
	...
	People are selected through an \textindex {LDAP filter} defined in a configuration file. This file must have the extension '.ldap'.It is stored in \dir {/usr/local/sympa-ff//etc/search\_filters/}.
...
\textit {Example :} 
\begin {quote}
/usr/local/sympa-ff//etc/\samplerobot/scenari/send.intranetorprivate:ignore
\end {quote}
...
\begin {quote}
\begin{verbatim}
sympa@your.virtual.domain:      "| /usr/local/sympa-ff//bin/queue sympa@your.virtual.domain"
listmaster@your.virtual.domain: "| /usr/local/sympa-ff//bin/queue listmaster@your.virtual.domain"
\end{verbatim}
\end {quote}
...
\dir {/usr/local/sympa-ff//etc/\samplerobot}. This directory must contain at least a 
\file {robot.conf} file. This files has the same format as  \file {/usr/local/sympa-ff//etc/sympa.conf}
...
These settings overwrite the equivalent global parameter defined in \file {/usr/local/sympa-ff//etc/sympa.conf}
...
\dir {/usr/local/sympa-ff//etc/\samplerobot/wws\_templates/},
\dir {/usr/local/sympa-ff//etc/\samplerobot/templates/}, 
\dir {/usr/local/sympa-ff//etc/\samplerobot/scenari/} directories are searched when
loading templates or scenari before searching into \dir {/usr/local/sympa-ff//etc} and  \dir {/usr/local/sympa-ff//bin/etc}. This allows to define different privileges and a different GUI for a Virtual Robot.
...
robot to a dedicated virtual robot located in the \dir {/usr/local/sympa-ff//expl/\samplerobot/} directory. The main benefit of 
...
To create such a virtual robot, you need to create \dir {/usr/local/sympa-ff//expl/\samplerobot/} and \file {/usr/local/sympa-ff//etc/\samplerobot/} directories ; 
customize \cfkeyword {host}, \cfkeyword {http\_host} and \cfkeyword {wwsympa\_url} parameters in the \file {/usr/local/sympa-ff//etc/\samplerobot/robot.conf} 
...
\begin {enumerate}
	\item \dir {/usr/local/sympa-ff//expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//etc/\samplerobot/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//etc/\samplerobot/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//bin/etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}.
	\item \dir {/usr/local/sympa-ff//bin/etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}.
\end {enumerate}
...
Your list web template files should be placed in the \dir {/usr/local/sympa-ff//expl/\samplelist/web\_tt2} 
directory ; your site web templates in \tildedir {/usr/local/sympa-ff//etc/web\_tt2} directory.
...
\dir {/usr/local/sympa-ff//locale} directory. 
...
file, located in the \dir {/usr/local/sympa-ff//etc} directory. The format of this file is 
as follows :
\begin {quote}
\begin{verbatim}
<topic1_name>
...
\Sympa provides several task models stored in \dir {/usr/local/sympa-ff//bin/etc/global\_task\_models} 
  and \dir {/usr/local/sympa-ff//bin/etc/list\_task\_models} directories.
...
\begin {enumerate}
	\item \dir {/usr/local/sympa-ff//expl/\texttt{<}list name\texttt{>}/}
	\item \dir {/usr/local/sympa-ff//etc/list\_task\_models/}
	\item \dir {/usr/local/sympa-ff//bin/etc/list\_task\_models/}
\end {enumerate}
...
\begin {enumerate}
	\item \dir {/usr/local/sympa-ff//etc/global\_task\_models/}
	\item \dir {/usr/local/sympa-ff//bin/etc/global\_task\_models/}
\end {enumerate}
...
\file {/usr/local/sympa-ff//expl/\samplerobot/\samplelist/config} 
(or \file {/usr/local/sympa-ff//expl/\samplelist/config} if no virtual robot is defined). 
...
The \file {/usr/local/sympa-ff//expl/\samplelist/subscribers} file is automatically created and
...
\file {/usr/local/sympa-ff//expl/\samplelist/info} should contain a detailed text
...
\file {/usr/local/sympa-ff//expl/\samplelist/homepage} is the HTML text 
...
\begin {enumerate}
 	\item \dir {/usr/local/sympa-ff//expl/\samplelist/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/usr/local/sympa-ff//etc/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/usr/local/sympa-ff//etc/\samplerobot/data\_sources/\texttt{<}file\texttt{>}.incl}.
\end {enumerate} 
...
\begin {enumerate}
 	\item \dir {/usr/local/sympa-ff//expl/\samplelist/mail\_tt2/\texttt{<}file\texttt{>}.tt2} 
	\item \dir {/usr/local/sympa-ff//etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}. 
	\item \dir {/usr/local/sympa-ff//bin/etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}.
\end {enumerate}
...
\file {/usr/local/sympa-ff//expl/\samplelist/stats} is a text file containing 
...
You may create \file {/usr/local/sympa-ff//expl/\samplelist/message.header} and
\file {/usr/local/sympa-ff//expl/\samplelist/message.footer} files. Their content
...
The \dir {/usr/local/sympa-ff//expl/\samplelist/archives/} directory contains the 
...
List profiles are stored in \dir {/usr/local/sympa-ff//etc/create\_list\_templates} or in
\dir {/usr/local/sympa-ff//bin/etc/create\_list\_templates} (default of distrib).
...
\dir {/usr/local/sympa-ff//etc/create\_list\_templates} and
\dir {/usr/local/sympa-ff//bin/etc/create\_list\_templates} directory, then the local profile 
...
the \dir {/usr/local/sympa-ff//bin/etc} directory, and you may create your own customized
version in \dir {/usr/local/sympa-ff//etc}).
...
For each parameter, you may specify (via the \file {/usr/local/sympa-ff//etc/edit\_list.conf}
configuration file) who has the right to edit the parameter concerned ; the default 
\file {/usr/local/sympa-ff//bin/etc/edit\_list.conf} is reasonably safe.
...
Domain name of the list, default is the robot domain name set in the related \file {robot.conf} file or in file \file {/usr/local/sympa-ff//etc/sympa.conf}.
...
is used otherwise in \dir {/usr/local/sympa-ff//etc}.
\end{itemize}
...
Maximum size of a message in 8-bit bytes. The default value is set in
 the \file {/usr/local/sympa-ff//etc/sympa.conf} file.
...
\item  \lparam {footer\_type} \texttt {append} 
        ...
        message. \file {/usr/local/sympa-ff//expl/\samplelist/message.footer.mime} will be
...
they are stored in \dir {/usr/local/sympa-ff//expl/\samplelist/archives/} directory.
...
(\dir {/usr/local/sympa-ff//expl/\samplelist/archives/}), used to store these documents.
...
Tagging a message will create a topic information file in the \dir {/usr/local/sympa-ff//spool/topic/} spool. Its name is based on the listname and the Message-ID. 
...
these messages are saved in the \dir {/usr/local/sympa-ff//spool/tracability/} spool if authorization or authentication are needed. Afterward, they moved in the \dir {tracability/} directory of 
...
A list's shared documents are stored in the \dir {/usr/local/sympa-ff//expl/\samplelist/shared}
directory. 
...
\begin {quote}
\begin{verbatim}
	\samplelist-owner: "|/usr/local/sympa-ff//bin/bouncequeue \samplelist"
\end{verbatim}
\end {quote}

\file {bouncequeue} (see \ref{binaries}, page~\pageref{binaries}) stores bounces in a
\dir {/usr/local/sympa-ff//spool/bounce/} spool.
...
For each mail received, \Sympa extracts its MIME parts in the \dir {/usr/local/sympa-ff//spool/tmp/antivirus} directory and
...
\begin{itemize}
  ...
  \item \cfkeyword {cafile} : the path of a bundle of trusted ca certificates. 
        The file \tildefile {/usr/local/sympa-ff//bin/etc/ca\-bundle.crt} included in Sympa distribution can be used.
...
the listmaster using \dir {/usr/local/sympa-ff//bin/p12topem.pl} script. This script allows
...
\dir {/usr/local/sympa-ff//expl/\samplelist/private\_key}. All the list private
...
the \unixcmd {/usr/local/sympa-ff//bin/p12topem.pl} script. Use \unixcmd
...
certificates stored in the \dir {/usr/local/sympa-ff//expl/X509-user-certs/} directory.
...
Tags the message by creating its topic information file in the \dir {/usr/local/sympa-ff//spool/topic/} spool. 
...
Researches the tracability files .sub for subscription or .auth for authentication in the \dir{/usr/local/sympa-ff//spool/tracability/} spool. 
...
Deletes the tracability files .sub for subscription or .auth for authentication in the \dir{/usr/local/sympa-ff//spool/tracability/} spool.
