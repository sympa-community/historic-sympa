...
[ELSE]
  ...
  [IF path]  
    ...
    <A HREF="[path_cgi]/d_delete/[list]/[escaped_path]" onClick="request_confirm_link('[path_cgi]/d_delete/[list]/[escaped_path]', '確定要刪除 [path] ?'); return false;">刪除</A> |
    ...
    描述: [doc_title] <BR><BR>
    [ENDIF]
    <font size=+1> <A HREF="[path_cgi]/d_read/[list]/[father]"> <IMG ALIGN="bottom"  src="[father_icon]"> 轉到上一級目錄</A></font>
    <BR>  
  [ELSE]
    <h2> <B> 文件夾 SHARED 的內容 </B> </h2> 
  [ENDIF]
   
  <TABLE width=100%>
  <TR BGCOLOR="[dark_color]">
   
  <th><TABLE width=100%><TR><TD ALIGN="left"><font color="[bg_color]">文檔</font></TD>
  ...
  <th><TABLE width=100%><TR><TD ALIGN="left"><font color="[bg_color]">最後更新</font></TD>
  ...
  <th ALIGN="center"><font color="[bg_color]">編輯</font></th> 
  <th ALIGN="center"><font color="[bg_color]">刪除</font></th>
  <th ALIGN="center"><font color="[bg_color]">存取</font></th></TR>
      
  [IF empty]
    <TR BGCOLOR="[light_color]" VALIGN="top">
    <TD COLSPAN=8 ALIGN="center"> 空文件夾 </TD>
    </TR>
  [ELSE]   
    [IF sort_subdirs]
	...
	<A HREF="[path_cgi]/d_editfile/[list]/[escaped_path][s->escaped_doc]">修改</A>
	  ...
	  <TD><center>
          <FONT size=-1>
          <A HREF="[path_cgi]/d_delete/[list]/[escaped_path][s->escaped_doc]" onClick="request_confirm_link('[path_cgi]/d_delete/[list]/[escaped_path][s->escaped_doc]', '您確定要刪除 [path][s->doc] 嗎 ?'); return false;">刪除</A>
	  </FONT>
	  </center></TD>
	[ELSE]
	  <TD>&nbsp; </TD>
	...
	<center>
	<FONT size=-1>
	<A HREF="[path_cgi]/d_delete/[list]/[escaped_path][f->escaped_doc]" onClick="request_confirm_link('[path_cgi]/d_delete/[list]/[escaped_path][f->escaped_doc]', '您確定要刪除 [path][s->doc] ([f->size] Kb) 嗎?'); return false;">刪除</A>
	</FONT>
	</center>
	</TD>
	[ELSE]
...
<TABLE CELLSPACING=20>
  ...
  [IF may_edit]
    ...
    [IF path]
      <B> 在文件夾 [path] 裡建立新文件夾</B> <BR>
    [ELSE]
      <B> 在文件夾 SHARED 裡建立新文件夾</B> <BR>
    [ENDIF]
    <input MAXLENGTH=30 type="text" name="name_doc">
    </TD>

    <TD ALIGN="left" VALIGN="bottom">
    <input type="submit" value="建立新的子目錄" name="action_d_create_dir">
   ...
   <TD ALIGN="right" VALIGN="bottom">
   [IF path]
     <B> 上傳一個文件到文件夾 [path] 中 </B><BR>
   [ELSE]
     <B> 上傳一個文件到文件夾 SHARED 中 </B><BR>
   [ENDIF]
   <input type="file" name="uploaded_file">
   </TD>

   <TD ALIGN="left" VALIGN="bottom">
   <input type="submit" value="發布" name="action_d_upload">
   <INPUT TYPE="hidden" NAME="list" VALUE="[list]">
   <INPUT TYPE="hidden" NAME="path" VALUE="[path]">
   <TD>
   </form> 
