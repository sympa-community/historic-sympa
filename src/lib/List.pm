...
sub db_connect {
    ...
    ##    load()-ed later.
    if ($Conf::Conf{'db_type'} eq 'Oracle' or
	$Conf::Conf{'db_type'} eq 'Sybase') {
	$dbh->{LongReadLen} = $Conf::Conf{'max_size'} * 2;
	$dbh->{LongTruncOk} = 0;
    }
    do_log('debug3',
	'Database driver seetings for this session: LongReadLen= %d, LongTruncOk= %d, RaiseError= %d',
	$dbh->{LongReadLen}, $dbh->{LongTruncOk}, $dbh->{RaiseError});
...
sub load {
    ...
    if (
        &Conf::get_robot_conf($self->{'domain'}, 'cache_list_config') eq 'binary_file'
...
sub send_file {
    ...
    if ((&Conf::get_robot_conf($self->{'domain'}, 'dkim_feature') eq 'on')&&(&Conf::get_robot_conf($self->{'domain'}, 'dkim_add_signature_to')=~/list/)){
...
sub is_web_archived {
    my $self = shift;
    return 1 if (ref $self->{'admin'}{'web_archive'} eq 'HASH' and $self->{'admin'}{'web_archive'}{'access'});
    return undef;
}
