...
sub connect_sympa_database {
    ...
    &Log::do_log('debug3', 'Connected to Database %s', Conf->db_name);
...
sub db_disconnect {
	...
	&Log::do_log('err','Can\'t disconnect from Database %s : %s',Conf->db_name, $db_source->{'dbh'}->errstr);
	return undef;
    }
...
sub probe_db {
	...
	unless ($found) {
		...
		push @report, $rep;
		&Log::do_log('notice', 'Table %s created in database %s', $t1, Conf->db_name);
		push @tables, $t1;
		$real_struct{$t1} = {};
	    }
	}
	...
	foreach my $t (keys %{$db_struct{'mysql'}}) {
		...
		&Log::do_log('err', "Table '%s' not found in database '%s' ; you should create it with create_db.%s script", $t, Conf->db_name, Conf->db_type);
		return undef;
	    }
	    ...
	    if (Conf->db_type eq 'mysql' or Conf->db_type eq 'Pg' or
		Conf->db_type eq 'SQLite') {
    ...
    ## Notify listmaster
    &List::send_notify_to_listmaster('db_struct_updated', Conf->domain,
				     {'report' => \@report})
	if scalar @report;
...
sub check_fields {
	...
	unless ($real_struct{$t}{$f}) {
	    push @{$report_ref}, sprintf("Field '%s' (table '%s' ; database '%s') was NOT found. Attempting to add it...", $f, $t, Conf->db_name);
	    &Log::do_log('info', "Field '%s' (table '%s' ; database '%s') was NOT found. Attempting to add it...", $f, $t, Conf->db_name);

	    my $rep;
	    if ($rep = $db_source->add_field({
		'table' => $t,
		'field' => $f,
		'type' => $db_struct{Conf->db_type}{$t}{$f},
		'notnull' => $not_null{$f},
	...
	if (Conf->update_db_field_types eq 'auto') {
		...
		push @{$report_ref}, sprintf("Field '%s'  (table '%s' ; database '%s') does NOT have awaited type (%s). Attempting to change it...",$f, $t, Conf->db_name, $db_struct{Conf->db_type}{$t}{$f});
		
		&Log::do_log('notice', "Field '%s'  (table '%s' ; database '%s') does NOT have awaited type (%s) where type in database seems to be (%s). Attempting to change it...",$f, $t, Conf->db_name, $db_struct{Conf->db_type}{$t}{$f},$real_struct{$t}{$f});
		...
		&Log::do_log('err', 'Field \'%s\'  (table \'%s\' ; database \'%s\') does NOT have awaited type (%s).', $f, $t, Conf->db_name, $db_struct{Conf->db_type}{$t}{$f});
