...
sub create {
    ...
    Sympa::Log::Syslog::do_log('debug', '(%s, %s, %s)', undef, $email, $robot);
...
sub load {
    ...
    Sympa::Log::Syslog::do_log('debug', '(%s)', $id_challenge);

    unless ($id_challenge) {
	Sympa::Log::Syslog::do_log('err', 'internal error, Sympa::Session::load called with undef id_challenge');
	return undef;
    }
	...
	Sympa::Log::Syslog::do_log('err','Unable to retrieve challenge %s from database',$id_challenge);
	return undef;
    }
    ...
    Sympa::Log::Syslog::do_log('debug3', 'removing existing challenge del_statement = %s',$id_challenge);
