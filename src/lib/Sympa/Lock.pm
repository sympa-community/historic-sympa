...
sub lock {
    ...
    if ($list_of_locks{$self->{'lock_filename'}}{'fh'}) {
        ...
        if (    $mode eq 'write'
            and $list_of_locks{$self->{'lock_filename'}}{'mode'} eq 'read') {
            Sympa::Log::Syslog::do_log('debug3',
                'Need to unlock and redo locking on %s', $self);
            ...
            if ($self->add_lock($mode, -1)) {
                push @{$list_of_locks{$self->{'lock_filename'}}
                        {'states_list'}},
                    $mode;
            } else {
...
sub unlock {
    ...
    unless (defined $list_of_locks{$self->{'lock_filename'}}) {
        Sympa::Log::Syslog::do_log('err',
            'Failed to unlock file %s ; file is not locked', $self);
        return undef;
    }
    my $previous_mode;
    my $current_mode;
    ...
    if ($#{$list_of_locks{$self->{'lock_filename'}}{'states_list'}} > 0) {
        ...
        $current_mode =
            @{$list_of_locks{$self->{'lock_filename'}}{'states_list'}}
            [$#{$list_of_locks{$self->{'lock_filename'}}{'states_list'}}];
...
sub remove_lock {
    ...
    if ($lock_method eq 'nfs') {
        ...
        unless (defined $fh
            and defined $nfs_lock
            and _unlock_nfs($self->{'lock_filename'}, $fh, $nfs_lock)) {
...
sub _lock_file {
    ...
    unless (flock($fh, $operation | LOCK_NB)) {
        if ($timeout == -1) {
            Sympa::Log::Syslog::do_log('err',
                'Unable to get a new lock and other locks pending in this process. Cancelling.'
            );
            return undef;
        }
        ...
        if ((time - (stat($lock_file))[9]) >= $timeout) {
            Sympa::Log::Syslog::do_log('debug3', 'Removing lock file %s',
                $lock_file);
    ...
    if ($got_lock) {
        Sympa::Log::Syslog::do_log('debug3', 'Got lock for %s on %s',
            $mode, $lock_file);
