    ...
    if ($param->{'db_type'} =~ /^mysql$/i) {
        unless (eval "require Sympa::DatabaseDriver::MySQL") {
            $main::logger->do_log(Sympa::Logger::ERR,
                "Unable to use Sympa::DatabaseDriver::MySQL module: $EVAL_ERROR");
            return undef;
        }
        ...
        unless (eval "require Sympa::DatabaseDriver::Postgres") {
            $main::logger->do_log(Sympa::Logger::ERR,
                "Unable to use Sympa::DatabaseDriver::Postgres module");
            return undef;
        }
        require Sympa::DatabaseDriver::Postgres;

        $actualclass = "Sympa::DatabaseDriver::Postgres";
...
sub connect {
    my $self = shift;
    $main::logger->do_log(Sympa::Logger::DEBUG3, "Checking connection to database %s",
        $self->{'db_name'});
...
sub establish_connection {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG, 'Creating connection to database %s',
        $self->{'db_name'});
    ...
    if (defined $db_connections{$self->{'connect_string'}}
        ...
        $main::logger->do_log(Sympa::Logger::DEBUG3, 'Use previous connection');
...
sub do_query {
    ...
    my $s = $statement;
    $s =~ s/\n\s*/ /g;
    $main::logger->do_log(Sympa::Logger::DEBUG2, "Will perform query '%s'", $s);
...
sub do_prepared_query {
    ...
    $query =~ s/\s+$//;
    $query =~ s/\n\s*/ /g;
    $main::logger->do_log(Sympa::Logger::DEBUG3, "Will perform query '%s'", $query);
