...
sub distribute_msg {
	...
	if ($info_msg_topic) {
	    my $topicspool = Sympa::Spool->newClassic('topic');
...
sub send_msg_digest {
    ...
    my $digestspool = Sympa::Spool->new('digest', undef,
...
sub may_edit {
	...
	my $edit_conf_file = Sympa::Tools::get_filename('etc',{},'edit_list.conf',$self->{'domain'},$self, Sympa::Site->etc);
...
sub archive_msg {
	...
	if (
	    Sympa::Site->ignore_x_no_archive_header_feature ne 'on' and
	    ...
	    my $spoolarchive = Sympa::Spool->newClassic('outgoing');
