...
sub get_https2{
	...
	my $trusted_ca_file = $ssl_data->{'cafile'};
	$trusted_ca_file ||= $Sympa::Configuration::Conf{'cafile'};
	my $trusted_ca_path = $ssl_data->{'capath'};
	$trusted_ca_path ||= $Sympa::Configuration::Conf{'capath'};

	&Sympa::Log::do_log ('debug','Fetch::get_https2 (%s,%s,%s,%s,%s)',$host,$port,$path,$trusted_ca_file,$trusted_ca_path );
	...
	&Sympa::Log::do_log ('debug',"connected to https://$host:$port/");
...
#	}
	...
	&Sympa::Log::do_log ('debug',"requesting  $request");
	#my ($buffer) = $ssl_socket->getlines;
	# print STDERR $buffer;
	#&Sympa::Log::do_log ('debug',"return");
	#return ;

	&Sympa::Log::do_log ('debug',"get_https reading answer returns :");
	my @result;
	while (my $line = $ssl_socket->getline) {
	    &Sympa::Log::do_log ('debug',"$line");
	    push  @result, $line;
	} 
