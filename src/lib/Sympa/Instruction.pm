...
Sympa::Instruction - A task instruction

=head1 DESCRIPTION

This class implements a single instruction from a background task.
...
sub _parse {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG2, 'Parsing "%s"',
        $self->{'line_as_string'});
...
sub _chk_cmd {
    ...
    $main::logger->do_log(
        Sympa::Logger::DEBUG2, 'chk_cmd(%s, %d, %s)',
        $self->{'command'},
        $self->{'line_number'},
    ...
    if ($task->{'object'} eq '_global') {
        foreach my $email (keys %{$params->{$var}}) {
            ...
            croak "Unable to send template $template to $email\n"
                unless $result;
            $main::logger->do_log(Sympa::Logger::NOTICE, "--> message sent to $email");
        }
    } else {
...
sub _next_cmd {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG2, "--> new task $model ($human_date)");
    return 1;
}
...
sub _purge_logs_table {
    ...
    $main::logger->do_log(Sympa::Logger::NOTICE, 'purge_logs_table(): logs purged');
    return 1;
}
...
sub _purge_tables {
    ...
    $main::logger->do_log(Sympa::Logger::INFO, 'task_manager::purge_tables()');
    ...
    $main::logger->do_log(Sympa::Logger::NOTICE, '%s rows removed in bulkspool_table',
        $removed);
    ...
    $main::logger->do_log(Sympa::Logger::NOTICE, "%s rows removed in tracking table",
        $removed);
...
sub _purge_one_time_ticket_table {
    ...
    my $removed = Sympa::Session::purge_old_tickets(undef, $delay);
...
sub _purge_user_table {
    ...
    foreach (@users) {
        unless ($known_people{$_}) {
            $main::logger->do_log(Sympa::Logger::DEBUG2, 'User to purge: %s', $_);
            push @purged_users, $_;
        }
    }
...
sub _purge_orphan_bounces {
    ...
    $main::logger->do_log(Sympa::Logger::INFO, 'purge_orphan_bounces()');
    ...
    unless ($all_lists = Sympa::List::get_lists()) {
        $main::logger->do_log(Sympa::Logger::NOTICE, 'No list available');
        return 1;
    }
...
sub _expire_bounce {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG2, 'expire_bounce(%d)', $delay);
...
sub _chk_cert_expiration {
    ...
    foreach (@certificates) {
        ...
        if (   ($expiration_date > $execution_date)
            ...
            $id =~ s/subject= //;
            $main::logger->do_log(Sympa::Logger::NOTICE, "id : $id");
...
sub _update_crl {
    ...
    unless (-d $crl_dir) {
        ...
        $main::logger->do_log(Sympa::Logger::NOTICE, 'creating spool %s', $crl_dir);
    }
...
sub _eval_bouncers {
    ...
    foreach my $list (@$all_lists) {
        ...
        $main::logger->do_log(Sympa::Logger::INFO, 'eval_bouncers(%s)', $listname);
...
sub _get_score {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG, 'Get_score(%s) ',
        $user_ref->{'email'});
...
sub _sync_include {
    ...
    $main::logger->do_log(Sympa::Logger::DEBUG2, 'sync_include(%s)', $task->{'id'});
