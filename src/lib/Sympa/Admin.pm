...
sub create_list_old {
    ...
    unless (open INFO, '>', "$list_dir/info") {
        Sympa::Log::Syslog::do_log('err', 'Impossible to create %s/info : %s',
            $list_dir, $!);
    }
...
sub create_list {
    ...
    foreach my $arg ('listname') {
        unless ($param->{$arg}) {
            Sympa::Log::Syslog::do_log('err', 'missing list param "%s"',
                $arg);
            return undef;
        }
    }
    ...
    unless (defined $template_file) {
        Sympa::Log::Syslog::do_log('err', 'no config template from family %s',
            $family);
        return undef;
    }
    ...
    my $list;
    unless ($list = new List($param->{'listname'}, $robot)) {
        Sympa::Log::Syslog::do_log('err', 'unable to create list %s',
            $param->{'listname'});
        return undef;
    }
...
sub clone_list_as_empty {
    ...
    unless ($new_list =
        new List($new_listname, $new_robot_id, {'reload_config' => 1})) {
        Sympa::Log::Syslog::do_log('info',
...
sub install_aliases {
    ...
    if ($status == 1) {
        ...
        Sympa::Log::Syslog::do_log('err', 'Unable to run newaliases: %s',
            $error_output);
    } elsif ($status == 7) {
        Sympa::Log::Syslog::do_log('err',
