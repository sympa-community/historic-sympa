...
sub new {
	...
	if (defined $params{'file_path'}) {
		...
		unless ($filename =~ /^([^\.]+)\.(.+)$/) {
			Sympa::Log::Syslog::do_log('err',"Failed to determine scenario type and name from '$params{'file_path'}'");
			return undef;
		}
		...
		foreach my $dir (@dirs) {
			...
			if (-r $tmp_path) {
				...
				Sympa::Log::Syslog::do_log('debug','Unable to read file %s',$tmp_path);
			}
		}
	}
	...
	if (defined $self->{'file_path'}) {
		...
		unless (open SCENARIO, $self->{'file_path'}) {
			Sympa::Log::Syslog::do_log('err',"Failed to open scenario '$self->{'file_path'}'");
			return undef;
		}
		...
		Sympa::Log::Syslog::do_log('err',"Unable to find scenario file '$params{'function'}.$params{'name'}', please report to listmaster");
...
sub _parse_scenario {
	...
	foreach my $current_rule (@rules) {
		...
		if ($current_rule =~ /\s*(include\s*\(?\'?(.*)\'?\)?)\s*$/i) {
			...
			Sympa::Log::Syslog::do_log('err', "error rule syntaxe in scenario
				$function rule line $NR expected : <condition> <auth_mod> -> <action>");
			Sympa::Log::Syslog::do_log('err',"error parsing $current_rule");
			return undef;
		}
...
sub request_action {
	...
	if ($log_it) {
		if ($context->{'list_object'}) {
			...
			Sympa::Log::Syslog::do_log('info','Will evaluate scenario %s for list %s@%s',$operation,$context->{'list_object'}{'name'},$robot);
		} else {
			$trace_scenario = 'scenario request '.$operation.' for robot '.$robot.' :';
			Sympa::Log::Syslog::do_log('info','Will evaluate scenario %s for robot %s',$operation,$robot);
		}
	}
	...
	unless ($name) {
		Sympa::Log::Syslog::do_log('err',"internal error : configuration for operation $operation is not yet performed by scenario");
		return undef;
	}
	...
	foreach my $rule (@rules) {
		...
		if ($auth_method eq $rule->{'auth_method'}) {
			...
			## Cope with errors
			if (! defined ($result)) {
				Sympa::Log::Syslog::do_log('info',"error in $rule->{'condition'},$rule->{'auth_method'},$rule->{'action'}" );
				Sympa::Log::Syslog::do_log('info', 'Error in %s scenario, in list %s', $context->{'scenario'}, $context->{'listname'});
				...
				unless (Sympa::List::send_notify_to_listmaster('error-performing-condition', $robot, [$context->{'listname'}."  ".$rule->{'condition'}] )) {
					Sympa::Log::Syslog::do_log('notice',"Unable to send notify 'error-performing-condition' to listmaster");
				}
				return undef;
			}
			...
			if ($result == 1) {
				...
				unless ($action =~ /^(do_it|reject|request_auth|owner|editor|editorkey|listmaster|ham|spam|unsure)/) {
					Sympa::Log::Syslog::do_log('err', "Matched unknown action '%s' in scenario", $rule->{'action'});
					return undef;
				}
				return $return;
			}
...
#	Sympa::Log::Syslog::do_log('debug3',"verify: context->{$k} = $v");
#    }

	unless (defined($context->{'sender'} )) {
		Sympa::Log::Syslog::do_log('info',"internal error, no sender find, report authors");
		return undef;
	}
	...
	while ($arguments =~ s/^\s*(
		...
		## Config param
			...
			if (my $conf_value = Sympa::Configuration::get_robot_conf($robot, $1)) {
				...
				Sympa::Log::Syslog::do_log('debug',"undefine variable context $value in rule $condition");
				if ($log_it == 1) {
					Sympa::Log::Syslog::do_log('info',"undefine variable context $value in rule $condition");
				}
			...
			if ($param =~ /^(name|total)$/) {
				...
				Sympa::Log::Syslog::do_log('err','Unknown list parameter %s in rule %s', $value, $condition);
				if ($log_it == 1) {
					Sympa::Log::Syslog::do_log('info','Unknown list parameter %s in rule %s', $value, $condition);
				}
				return undef;
			}
			...
			if (defined ($context->{'msg'})) {
				...
				if ($log_it == 1) {
					Sympa::Log::Syslog::do_log('info','no message object found to evaluate rule %s', $condition);
				}
				return -1 * $negation;
			}
			...
			unless (defined ($context->{'msg'})) {
				if ($log_it == 1) {
					Sympa::Log::Syslog::do_log('info','no message to evaluate rule %s', $condition);
				}
				return -1 * $negation;
			}
	...
	if ($condition_key =~ /^(true|any|all)$/i) {
		if ($log_it == 1) {
			Sympa::Log::Syslog::do_log('info','Condition %s is always true (rule %s)',$condition_key,$condition);
		}
		return $negation;
	}
	...
	if ($condition_key eq 'is_listmaster') {
		...
		if ( Sympa::List::is_listmaster($args[0],$robot)) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','%s is listmaster of robot %s (rule %s)',$args[0],$robot,$condition);
			}
			return $negation;
		} else {
			if ($log_it == 1) {
...
##### condition verify_netmask
	if ($condition_key eq 'verify_netmask') {
		...
		unless (defined $ENV{'REMOTE_ADDR'}) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','REMOTE_ADDR env variable not set (rule %s)',$condition);
			}
			return -1; ## always skip this rule because we can't evaluate it
		}
		my $block;
		unless ($block = new2 Net::Netmask ($args[0])) {
			Sympa::Log::Syslog::do_log('err', "error rule syntaxe : failed to parse netmask '$args[0]'");
			return undef;
		}
		if ($block->match ($ENV{'REMOTE_ADDR'})) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','REMOTE_ADDR %s matches %s (rule %s)',$ENV{'REMOTE_ADDR'},$args[0],$condition);
			}
			return $negation;
		} else {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','REMOTE_ADDR %s does not match %s (rule %s)',$ENV{'REMOTE_ADDR'},$args[0],$condition);
			}
			return -1 * $negation;
		}
	}
	...
	if ($condition_key =~ /^(older|newer)$/) {
		...
		if ($arg0 <= $arg1 ) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','%s is smaller than %s (rule %s)',$arg0, $arg1,$condition);
			}
			return $negation;
		} else {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info','%s is NOT smaller than %s (rule %s)',$arg0, $arg1,$condition);
			}
			return -1 * $negation;
		}
	}
	...
	if ($condition_key eq 'match') {
		unless ($args[1] =~ /^\/(.*)\/$/) {
			Sympa::Log::Syslog::do_log('err', 'Match parameter %s is not a regexp', $args[1]);
			return undef;
		}
		my $regexp = $1;
		...
		if ($regexp =~ /^$/) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info',"regexp '%s' is empty (rule %s)",$regexp,$condition);
			}
			return -1 * $negation;
		}
		...
		if ($EVAL_ERROR) {
			Sympa::Log::Syslog::do_log('err', 'cannot evaluate match: %s', $EVAL_ERROR);
			return undef;
		}
		if ($r) {
			if ($log_it == 1) {
				...
				Sympa::Log::Syslog::do_log('info',"'%s' matches regexp '%s' (rule %s)",$args_as_string,$regexp,$condition);
			}
			return $negation;
		} else {
			if ($log_it == 1) {
	...
	if ($condition_key eq 'search') {
		...
		if($val_search == 1) {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info',"'%s' found in '%s', robot %s (rule %s)",$context->{'sender'},$args[0],$robot,$condition);
			}
			return $negation;
		} else {
			if ($log_it == 1) {
				Sympa::Log::Syslog::do_log('info',"'%s' NOT found in '%s', robot %s (rule %s)",$context->{'sender'},$args[0],$robot,$condition);
			}
			return -1 * $negation;
		}
	}
		...
		if (ref($args[0])) {
			foreach my $arg (@{$args[0]}) {
				...
				if (lc($arg) eq lc($args[1])) {
					if ($log_it == 1) {
						Sympa::Log::Syslog::do_log('info',"'%s' equals '%s' (rule %s)",lc($arg),lc($args[1]),$condition);
					}
					return $negation;
				}
			}
			...
			if (lc($args[0]) eq lc($args[1])) {
				if ($log_it == 1) {
					Sympa::Log::Syslog::do_log('info',"'%s' equals '%s' (rule %s)",lc($args[0]),lc($args[1]),$condition);
				}
				return $negation;
			}
		}
	...
	if ($condition_key =~ /^customcondition::(\w+)/o ) {
		...
		unless (defined $res) {
			if ($log_it == 1) {
				...
				Sympa::Log::Syslog::do_log('info',"custom condition '%s' returned an undef value with arguments '%s' (rule %s)",$condition,$args_as_string,$condition);
			}
			return undef;
		}
		if ($log_it == 1) {
			...
			if ($res == 1) {
				Sympa::Log::Syslog::do_log('info',"'%s' verifies custom condition '%s' (rule %s)",$args_as_string,$condition,$condition);
			} else {
		...
		if (ref($args[0])) {
			foreach my $arg (@{$args[0]}) {
				...
				if (Sympa::Tools::Data::smart_lessthan($arg, $args[1])){
					if ($log_it == 1) {
						Sympa::Log::Syslog::do_log('info',"'%s' is less than '%s' (rule %s)",$arg, $args[1],$condition);
					}
					return $negation;
				}
			}
...
sub search {
	...
	Sympa::Log::Syslog::do_log('debug2', '(%s,%s,%s)', $filter_file, $sender, $robot);
	...
	if ($filter_file =~ /\.sql$/) {
		...
		while ($filter =~ /\[(\w+(\-\>[\w\-]+)?)\]/x) {
			...
			unless (defined $context->{$var}) {
				Sympa::Log::Syslog::do_log('err', "Failed to parse variable '%s' in filter '%s'", $var, $file);
				return undef;
			}

			if (defined $key) { ## Should be a hash
				unless (defined $context->{$var}{$key}) {
					Sympa::Log::Syslog::do_log('err', "Failed to parse variable '%s.%s' in filter '%s'", $var, $key, $file);
					return undef;
				}
...
#        $filter =~ s/\[sender\]/$sender/g;
		...
		unless (defined $ds && $ds->connect() && $ds->ping) {
			Sympa::Log::Syslog::do_log('notice','Unable to connect to the SQL server %s:%d',$sql_conf->{'db_host'}, $sql_conf->{'db_port'});
			return undef;
		}
		...
		my $res = $ds->fetch;
		$ds->disconnect();
		Sympa::Log::Syslog::do_log('debug2','Result of SQL query : %d = %s', $res->[0], $statement);
...
#	$filter =~ s/\[sender\]/$sender/g;
	...
	unless (defined $ds && ($ldap = $ds->connect())) {
		Sympa::Log::Syslog::do_log('err',"Unable to connect to the LDAP server
			'%s'", $params->{'ldap_host'});
		return undef;
	}
	...
	foreach my $file (@files) {
		Sympa::Log::Syslog::do_log('debug3', 'found file  %s', $file);
...
sub verify_custom {
	...
	Sympa::Log::Syslog::do_log('debug2', '(%s,%s,%s,%s)', $condition, $filter, $robot, $list);
	if (defined ($persistent_cache{'named_filter'}{$condition}{$filter}) &&
		...
		Sympa::Log::Syslog::do_log('notice', 'Using previous custom condition cache %s', $filter);
	...
	eval { require "$file"; };
	if ($EVAL_ERROR) {
		Sympa::Log::Syslog::do_log('err', 'Error requiring %s : %s (%s)', $condition, "$EVAL_ERROR", ref($EVAL_ERROR));
		return undef;
	}
	my $res;
	eval "\$res = CustomCondition::${condition}::verify(\@{\$args_ref});";
	if ($EVAL_ERROR) {
		Sympa::Log::Syslog::do_log('err', 'Error evaluating %s : %s (%s)', $condition, "$EVAL_ERROR", ref($EVAL_ERROR));
		return undef;
	}
