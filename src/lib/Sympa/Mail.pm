...
sub mail_file {
	...
	unless ($header_ok{'date'}) {
		...
		$tzoff = sprintf '%s%02d%02d', $sign, int($tzoff / 3600), int($tzoff / 60) % 60;
		Sympa::Language::push_lang('en');
		$headers .= 'Date: '.POSIX::strftime("%a, %d %b %Y %H:%M:%S $tzoff", localtime $now)."\n";
...
##  passed to reformat_message() separately then attached to reformatted
...
##  reformat_message() already includes sub-message(s).  Then:
##   - When a part has an `X-Sympa-Attach:' header field for internal use, new
##     style, reformat_message() attaches raw sub-message to reformatted
...
sub _fix_part($$$$) {
    ...
    if ($part->head()->get('X-Sympa-Attach')) { # Need re-attaching data.
		...
		my ($newbody, $newcharset, $newenc) = MIME::Charset::body_encode(
			Encode::decode('utf8', $wrap),
			$charset,
			Replacement => 'FALLBACK'
		);
		if ($newenc eq $enc and $newcharset eq $charset and $newbody eq $body) {
