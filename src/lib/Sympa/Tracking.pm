...
sub get_recipients_status {
	...
	# the message->head method return message-id including <blabla@dom> where mhonarc return blabla@dom that's why we test both of them
	    ...
	    &Sympa::Log::do_log('err','Unable to retrieve tracking informations for message %s, list %s@%s', $msgid, $listname, $robot);
            return undef;
        }
        my @pk_notifs;
...
sub remove_message_by_period{
	...
	&Sympa::Log::do_log('err','Unable to remove the tracking informations older than %s days for list %s@%s', $limit, $listname, $robot);
	return undef;
    }
